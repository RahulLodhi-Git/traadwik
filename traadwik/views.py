...
from .models import Jobs, PriceList, ProductCategory, Brand,product;
from email.message import EmailMessage;
from django.contrib import messages;
import smtplib;

EMAIL_ADDRESS = "contact@traadwik.com"
EMAIL_PASSWORD = "C@ntract#2022"

def index(request):
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    return render(request,'index.html',{'brandList':brandList,'ProductCat':ProductCat})

# contact
def contact(request):
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    messages.success(request,'Your information successfully sent. we will contact you very soon.! Thank You')
    ...
    if request.method=="POST":
        ...
        if len(name)<2 or len(email)<5 or len(phone)<10 or len(message)<25:
            ...
            # ---Sendding Email-----
            msg = EmailMessage()
            msg['Subject'] = f"Reg:{subject}"
            msg['From'] = f'Contact Request <{EMAIL_ADDRESS}>'
            msg['To'] = 'contact@traadwik.com'
            msg.set_content(f"\n{message}\n\n From {email}\nMoblie number{phone}")
            msg.add_alternative("""
...
<body>
                                                                            ...
                                                                            <tbody>
                                                                                <tr>
                                                                                    <td class="esd-block-text es-m-txt-c" align="center">
                                                                                        <h2>Hey Traadwik Electric Pvt. Ltd. <br></h2>
                                                                                    </td>
                                                                                </tr>
                                                                                <tr>
                                                                                    <td class="esd-block-text es-m-txt-c" align="center">
                                                                                ...
                                                                                <tr>
                                                                                    <td class="esd-block-button es-p20t es-p15b es-p10r es-p10l" align="center">
                                                                                        <p><strong>Email:- <em>
                                                                                        </em> {femail}, Phone:- {fphone}</strong></p>
                                                                                        <br/>
                                                                            ...
                                                                            <tbody>
                                                                                <tr>
                                                                                    <td class="esd-block-text es-p10b" align="center">
                                                                                        <p style="line-height: 150%;"><strong>This mail only for Traadwik Electric Pvt. Ltd.  Owner.</strong></p>
                                                                                    </td>
                                                                                </tr>
                                                                                <tr>
                                                                                    <td class="esd-block-text es-p10t es-p10b" align="center">
                                                                                        <p>© Traadwik Electric Pvt. Ltd. 2022<br></p>
                                                                                    </td>
                                                                                </tr>
                                                                            </tbody>
                                                                        </table>
    ...
    <div style="position: absolute; left: -9999px; top: -9999px; margin: 0px;"></div>
            ...
            with smtplib.SMTP_SSL('mail.traadwik.com', 465) as smtp:
                smtp.login(EMAIL_ADDRESS, EMAIL_PASSWORD)
                smtp.send_message(msg)
    return render(request,'contact.html',{'brandList':brandList,'ProductCat':ProductCat})

# About
def about(request):
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    return render(request,'about.html',{'brandList':brandList,'ProductCat':ProductCat})

def careers(request):
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    cList=Jobs.objects.all()
    return render(request, 'careers.html',{'cList':cList,'brandList':brandList,'ProductCat':ProductCat})

# Price List
def priceList(request):
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    pList=PriceList.objects.all()
    return render(request,'price-list.html',{'pList':pList,'brandList':brandList,'ProductCat':ProductCat})
...
def productDetail(request,slug):
    ...
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all();
    ProductMeta=ProductCategory.objects.filter(slug=slug)[0]
    productList=product.objects.filter(subcategory=productName)
    return render(request,'product-detail.html',{'slug':productName,'brandList':brandList,'ProductCat':ProductCat,'ProductMeta':ProductMeta,'productList':productList})

# Search
def search(request):
    rawQuery=request.GET['search-query']
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    if 'search-query' in request.GET:
        query = 'You searched for: %r' % request.GET['search-query']
        searchResult=product.objects.filter(name__icontains=rawQuery)
    else:
        query = 'You submitted an empty form.'
    print(query)
    return render(request,'search.html',{'query':query,'brandList':brandList,'ProductCat':ProductCat,'result':searchResult})

# adminLogin 
def adminLogin(request):
    brandList=Brand.objects.all();
    ProductCat=ProductCategory.objects.all()
    return render(request,'adminLogin.html',{'brandList':brandList,'ProductCat':ProductCat})


# dashboard
def dashboard(request):

    return render(request,'dashboard.html',{'brandList':brandList,'ProductCat':ProductCat})